...
const Sacred = artifacts.require('./SACREDMock.sol')
...
const sacredConfig = require('sacred-token')
...
contract('Governance', (accounts) => {
  ...
  let lockingPeriod
  let balanceProposer
  const cap = toBN(sacredConfig.sacred.cap)
  const tenThousandSacred = toBN(10).pow(toBN(18)).mul(toBN(10000))
  ...
  before(async () => {
    ...
    sacred = await Sacred.new(governanceExpectedAddr, duration.days(30), [
      { to: miningPublicKey, amount: cap.toString() },
    ])
    const governanceImplementation = await Governance.new()
    const calldata = governanceImplementation.contract.methods.initialize(sacred.address).encodeABI()
    ...
    await sacred.transfer(secondProposer, balanceProposer.div(toBN(2)), { from: miningPublicKey })
    await sacred.transfer(proposer, balanceProposer, { from: miningPublicKey })
    await sacred.setChainId(chainId)
    ...
    domain = {
      name: await sacred.name(),
      version: '1',
      chainId,
      verifyingContract: sacred.address,
    }
    snapshotId = await takeSnapshot()
  })
  beforeEach(async () => {
    await sacred.approve(governance.address, cap.div(toBN(4)), { from: proposer })
  ...
  describe('#propose', () => {
    ...
    it('fails if proposer does not have voting power', async () => {
      ...
      const tenThousandSacred = toBN(10).pow(toBN(18)).mul(toBN(999))
      await sacred.transfer(voterBob, tenThousandSacred, { from: miningPublicKey })

      await sacred.approve(governance.address, tenThousandSacred, { from: voterBob })

      await governance.lockWithApproval(tenThousandSacred, { from: voterBob })
      await governance
  ...
  describe('#castVote', () => {
    ...
    it('should work if there are multiple voters', async () => {
      ...
      const tenThousandSacred = toBN(10).pow(toBN(18)).mul(toBN(10000)) // todo
      await sacred.transfer(voterBob, tenThousandSacred, { from: miningPublicKey })
      await sacred.transfer(voterAlice, tenThousandSacred.mul(toBN(2)), { from: miningPublicKey })

      await sacred.approve(governance.address, tenThousandSacred, { from: voterBob })
      await sacred.approve(governance.address, tenThousandSacred.mul(toBN(2)), { from: voterAlice })

      await governance.lockWithApproval(tenThousandSacred, { from: voterBob })
      await governance.lockWithApproval(tenThousandSacred.mul(toBN(2)), { from: voterAlice })
      ...
      const proposal = await governance.proposals(id)
      proposal.forVotes.should.be.eq.BN(tenThousandSacred.mul(toBN(2)))
      proposal.againstVotes.should.be.eq.BN(votesCount.add(tenThousandSacred))
    })
    ...
    it('should be able to update number of votes count if the same decision is chosen after more tokens are locked', async () => {
      const voterBob = accounts[5]
      const tenThousandSacred = toBN(10).pow(toBN(18)).mul(toBN(10000)) // todo
      const fiveThousandSacred = tenThousandSacred.div(toBN(2))
      await sacred.transfer(voterBob, tenThousandSacred, { from: miningPublicKey })

      await sacred.approve(governance.address, tenThousandSacred, { from: voterBob })

      await governance.lockWithApproval(fiveThousandSacred, { from: voterBob })
      ...
      proposal.againstVotes.should.be.eq.BN(votesCount.add(fiveThousandSacred))
    ...
    it('locks tokens after vote', async () => {
      const voterAlice = accounts[7]
      await sacred.transfer(voterAlice, tenThousandSacred, { from: miningPublicKey })
      await sacred.approve(governance.address, tenThousandSacred, { from: voterAlice })
      await governance.lockWithApproval(tenThousandSacred, { from: voterAlice })
    ...
    it('does not reduce lock time', async () => {
      ...
      await sacred.approve(governance.address, tenThousandSacred, { from: voterAlice })
      await governance.lockWithApproval(tenThousandSacred, { from: voterAlice })
      await sacred.approve(governance.address, balanceProposer.div(toBN(2)), { from: secondProposer })
  ...
  describe('#lock', () => {
    ...
    it('calls approve if signature is valid', async () => {
      const chainIdFromContract = await sacred.chainId()
      ...
      const balanceBefore = await sacred.balanceOf(governance.address)
      ...
      const balanceAfter = await sacred.balanceOf(governance.address)

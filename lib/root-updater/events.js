...
const { getProvider, getRPCUrl} = require('../../src/utils')
const abi = new ethers.utils.AbiCoder()

async function getSacredTreesEvents(type, fromBlock, toBlock) {
  const eventName = type === action.DEPOSIT ? 'DepositData' : 'WithdrawalData'
  const sacredTree = await getSacredTrees()
...
async function getPendingEventHashes(type, from, to) {
  try {
    let calls = []
    const target = (await getSacredTrees()).address
...
async function getEvents(type) {
  ...
  const events = await getSacredTreesEvents(type, 0, 'latest')

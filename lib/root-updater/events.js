...
async function getPendingEventHashes(sacredTrees, type, from, to) {
  try {
    ...
    if(netId == "1" || netId == "42" || netId == "4" || netId == "5") {
    ...
    const multicall = new Multicall({ ethersProvider: provider, multicallCustomContractAddress: multicallAddress, tryAggregate: true });

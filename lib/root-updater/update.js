...
async function updateTree(sacredTrees, committedEvents, pendingEvents, type) {
  ...
  while (pendingEvents.length >= INSERT_BATCH_SIZE) {
    ...
    const basePath = INSERT_BATCH_SIZE === 256 ? './sacred-trees-snarks/BatchTreeUpdate_cpp/BatchTreeUpdate' : './sacred-trees-snarks-light/BatchTreeUpdate_cpp/BatchTreeUpdate'

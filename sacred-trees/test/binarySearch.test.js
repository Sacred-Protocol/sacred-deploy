...
describe('findArrayLength', () => {
  ...
  beforeEach(async function () {
    const [operator, sacredProxy] = await ethers.getSigners()
    ...
    sacredTrees = await SacredTrees.deploy(operator.address, publicArray.address, {
      depositsFrom: 3,
      depositsStep: 3,
      withdrawalsFrom: 2,
      withdrawalsStep: 2,
    })
    await sacredTrees.initialize(sacredProxy.address, publicArray.address)
  })

  it('should work for even array', async () => {
    const depositsLength = await sacredTrees.findArrayLengthMock(
      publicArray.address,
      'deposits(uint256)',
      4,
      2,
